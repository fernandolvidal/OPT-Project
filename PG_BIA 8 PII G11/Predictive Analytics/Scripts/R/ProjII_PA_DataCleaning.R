...
DataSet_pa<-mergedtable[,c("USERNAME","TICKET","REQUEST_DATE","REQUEST_SERVICE",
                             "REQUEST_DESC","RESPONSE_DESC")]

summary(DataSet_pa)

# Top-10 most frequent Tickets
sort(table(DataSet_pa$TICKET),decreasing = TRUE)[1:10]

# Exclude Request_Service "ReadMessages" and "ucTXL4EB7qCSDu8" and clean levels
DataSet_pa<-DataSet_pa[-which(DataSet_pa$REQUEST_SERVICE=="ReadMessages" | DataSet_pa$REQUEST_SERVICE=="ucTXL4EB7qCSDu8"),]
DataSet_pa$REQUEST_SERVICE<-as.character(DataSet_pa$REQUEST_SERVICE)
DataSet_pa$REQUEST_SERVICE<-as.factor(DataSet_pa$REQUEST_SERVICE)
str(DataSet_pa)

# Exclude tickets ticket=0 and ticket=1
DataSet_pa<-DataSet_pa[-which(DataSet_pa$TICKET=="0" | DataSet_pa$TICKET=="1"),]

## Features

# Time between requests for the same ticket - DateDiff
library(lubridate)
DataSet_pa$REQUEST_DATE<-ymd_hms(DataSet_pa$REQUEST_DATE)
DataSet_pa<-DataSet_pa[order(DataSet_pa$TICKET,DataSet_pa$REQUEST_DATE),]
DataSet_pa$DateDiff<-unlist(tapply(DataSet_pa$REQUEST_DATE, INDEX = DataSet_pa$TICKET, FUN = function(x) c(NA,diff(x))))

# Sub TrainDataSet and TestDataSet - concept model 
#DataSet_pa<-DataSet_pa[1:5000,]

## Eliminate observations with DateDiff < 3min and the same "REQUEST_DESC" in getScheds
DataSet_pa$RequestDiff<-0
DataSet_pa<-DataSet_pa[-which(DataSet_pa$REQUEST_SERVICE=="GetScheds"),]

#Bind DataSet_pa with Dataframe only with "getScheds" already full clean (datediff>180 & same station)
DataSet<-DataSet[,-c(7,8)]
DataSet_pa<-rbind(DataSet_pa,DataSet)

DataSet_pa<-DataSet_pa[,c("USERNAME","TICKET","REQUEST_DATE","REQUEST_SERVICE",
                       "REQUEST_DESC","RESPONSE_DESC")]

range(DataSet_pa$REQUEST_DATE)
DataSet_pa<-DataSet_pa[order(DataSet_pa$REQUEST_DATE),]

#for (i in 2:nrow(DataSet_pa)) {DataSet_pa$RequestDiff[i] <-ifelse(DataSet_pa$REQUEST_DESC[i-1]==DataSet_pa$REQUEST_DESC[i],1,0)}

#DataSet_pa<-DataSet_pa[-which(DataSet_pa$DateDiff < 180 & DataSet_pa$RequestDiff==1),]

# Time Features and weather features
...
trPerc<-0.7
nTrain<-as.integer(trPerc*nrow(m))

Tst<-m[(nTrain+1):nrow(m),]
Train<-m[1:nTrain,]
